...
Then, to check out the admin UI, go to http://localhost:3001/admin.
...
Use the top-right toggle to switch between chat and admin. The two upload areas in chat mean different things:
- `Archivos para RAG` sends files to the backend and indexes them.
- `Archivos locales` keeps files in memory and only injects them into the current message when enabled.
