...
class RAGAppContainerConfig(BaseModel):
    ...
    def to_docker_create_kwargs(self) -> dict:
        volume_config = self.volume_config or RAGAppVolumeConfig(name=self.name)
        return {
            ...
            "volumes": volume_config.to_container_create_kwargs(),
        }
...
def _get_default_app_environment(app_name: str) -> Dict[str, str]:
    return {
        ...
        "EMBEDDING_DIM": "1024",
        "CHAT_REQUEST_LIMIT_THRESHOLD": str(settings.chat_request_limit_threshold),
