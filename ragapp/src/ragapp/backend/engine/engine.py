...
def get_chat_engine(
    ...
    def get_context_window() -> int:
        try:
            metadata = getattr(Settings.llm, "metadata", None)
            ...
            pass
        return int(os.getenv("LLM_CONTEXT_WINDOW", "1048576"))
    ...
    if not getattr(Settings.llm.metadata, "is_function_calling_model", False):
        logger.info("LLM does not support function calling; using context chat engine")
        system_prompt = ChatConfig.get_config().custom_prompt
        if citation_prompt is not None:
            system_prompt = (
                f"{system_prompt}\n{citation_prompt}"
                if system_prompt
                else citation_prompt
            )
        return CondensePlusContextChatEngine(
            llm=Settings.llm,
            memory=ChatMemoryBuffer.from_defaults(
                token_limit=get_context_window() - 256
            ),
