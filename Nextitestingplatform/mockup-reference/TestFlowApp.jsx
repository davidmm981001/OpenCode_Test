...
const DEMO_SEED = (globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(36).slice(2, 10)}`)
  .replace(/-/g, "")
  .slice(0, 8)
  .toUpperCase();

const demoId = (prefix, index) => `${prefix}-${DEMO_SEED}-${String(index + 1).padStart(2, "0")}`;

// ── Seed Data (sanitized at creation) ────────────────────────
const SEED_PROJECTS = [
  { id: demoId("p1", 0), code: `BNK-${DEMO_SEED}`, name: `Core Banking API ${DEMO_SEED}`, client: `Banco Internacional ${DEMO_SEED}`, description: `Migración OSB → AWS API Gateway. Módulos de seguridad y conciliación Visa Direct. (${DEMO_SEED})`, language: "Java · Spring Boot", qaFramework: "Playwright + Cucumber", jiraKey: `BNK${DEMO_SEED}`, createdAt: "2025-01-15T10:00:00Z", stats: { requirements: 12, stories: 22, testCases: 84, automatedTests: 56, jiraSynced: 38 } },
  { id: demoId("p2", 1), code: `INS-${DEMO_SEED}`, name: `SaludSa — JAMS Migration ${DEMO_SEED}`, client: `SaludSa ${DEMO_SEED}`, description: `Migración Hangfire a JAMS. Módulo de Fees y Coberturas con motor de reglas. (${DEMO_SEED})`, language: ".NET 8 / C#", qaFramework: "Serenity BDD", jiraKey: `INS${DEMO_SEED}`, createdAt: "2025-02-01T09:00:00Z", stats: { requirements: 8, stories: 15, testCases: 60, automatedTests: 22, jiraSynced: 15 } },
  { id: demoId("p3", 2), code: `ESB-${DEMO_SEED}`, name: `OSB Migration Bolivariano ${DEMO_SEED}`, client: `Banco Bolivariano ${DEMO_SEED}`, description: `Modernización ESB. Análisis ESQL y COBOL batch a microservicios Spring Boot. (${DEMO_SEED})`, language: "COBOL / ESQL", qaFramework: "Selenium + Java", jiraKey: `ESB${DEMO_SEED}`, createdAt: "2025-03-01T08:00:00Z", stats: { requirements: 5, stories: 10, testCases: 40, automatedTests: 0, jiraSynced: 0 } },
];

const SEED_STORIES = [
  { id: `HU-001-${DEMO_SEED}`, projectId: demoId("p1", 0), title: `Como usuario autenticado, quiero transferir fondos entre cuentas propias ${DEMO_SEED}`, module: `Transferencias ${DEMO_SEED}`, description: `Como usuario bancario autenticado mediante OAuth 2.0 + MFA, quiero poder realizar transferencias de fondos entre mis cuentas registradas. (${DEMO_SEED})`, criteria: ["DADO que el usuario está autenticado y tiene saldo disponible, CUANDO ingresa monto válido, ENTONCES la transferencia se procesa en menos de 3 segundos.", "DADO que el monto excede el saldo, CUANDO intenta confirmar, ENTONCES el sistema retorna error E-4001.", "DADO que la transferencia es exitosa, ENTONCES se genera registro en módulo CORECALC.", "DADO que el servicio ESQL retorna RECHAZADO, ENTONCES se registra en log de auditoría.", "DADO un fallo de red, ENTONCES el sistema implementa rollback automático."], status: StoryStatus.GENERATED, jiraId: `BNK-${DEMO_SEED}`, jiraSyncStatus: SyncStatus.SYNCED },
  { id: `HU-002-${DEMO_SEED}`, projectId: demoId("p1", 0), title: `Como administrador, quiero configurar reglas de límites de transferencia por perfil ${DEMO_SEED}`, module: `Seguridad ${DEMO_SEED}`, description: `Como administrador del sistema, necesito configurar límites diarios, semanales y mensuales diferenciados por perfil de usuario para cumplir regulaciones SBS. (${DEMO_SEED})`, criteria: ["DADO que el admin modifica un límite, CUANDO guarda, ENTONCES el cambio aplica en máximo 5 minutos.", "DADO que se configura límite $0, ENTONCES el sistema rechaza con error de negocio.", "DADO un cambio en límites, ENTONCES el módulo ESQL TransfValidation recibe la regla actualizada."], status: StoryStatus.REVIEW, jiraId: null, jiraSyncStatus: SyncStatus.PENDING },
  { id: `HU-003-${DEMO_SEED}`, projectId: demoId("p1", 0), title: `Como usuario, quiero consultar historial de movimientos con filtros avanzados ${DEMO_SEED}`, module: `Consultas ${DEMO_SEED}`, description: `Como usuario bancario autenticado, quiero consultar mis movimientos filtrando por fecha, tipo y monto para llevar control de mis transacciones. (${DEMO_SEED})`, criteria: ["DADO que el usuario aplica filtros válidos, CUANDO confirma, ENTONCES el sistema retorna resultados en menos de 2 segundos.", "DADO que no hay movimientos en el rango, ENTONCES el sistema muestra mensaje informativo sin error.", "DADO un rango de fechas mayor a 12 meses, ENTONCES el sistema advierte y pagina automáticamente."], status: StoryStatus.PENDING, jiraId: null, jiraSyncStatus: SyncStatus.PENDING },
];

const SEED_TESTCASES = [
  { id: `TC-001-${DEMO_SEED}`, storyId: `HU-001-${DEMO_SEED}`, projectId: demoId("p1", 0), description: `Transferencia exitosa con saldo suficiente ${DEMO_SEED}`, preconditions: `Usuario autenticado con OAuth 2.0 + MFA\nCuenta origen con saldo >= $500`, steps: `1. Navegar a Transferencias\n2. Seleccionar cuenta origen\n3. Ingresar monto $500\n4. Confirmar con token\n5. Verificar confirmación`, expected: `Mensaje de éxito visible\nTiempo < 3 segundos\nSaldo actualizado en ambas cuentas`, type: TestCaseType.FUNCTIONAL, priority: Priority.HIGH, status: StoryStatus.GENERATED },
  { id: `TC-002-${DEMO_SEED}`, storyId: `HU-001-${DEMO_SEED}`, projectId: demoId("p1", 0), description: `Transferencia rechazada por saldo insuficiente (error E-4001) ${DEMO_SEED}`, preconditions: `Usuario autenticado\nSaldo cuenta origen $10`, steps: `1. Navegar a Transferencias\n2. Ingresar monto $500\n3. Confirmar operación`, expected: `Error E-4001 visible\nMensaje descriptivo al usuario\nSaldo sin modificación`, type: TestCaseType.NEGATIVE, priority: Priority.HIGH, status: StoryStatus.GENERATED },
  { id: `TC-003-${DEMO_SEED}`, storyId: `HU-001-${DEMO_SEED}`, projectId: demoId("p1", 0), description: `Rollback automático ante falla de red durante transferencia ${DEMO_SEED}`, preconditions: `Usuario autenticado\nSimulación de corte de red en paso 3`, steps: `1. Iniciar transferencia válida\n2. Simular timeout en confirmación\n3. Verificar estado de cuentas`, expected: `Saldo sin modificación\nRollback confirmado en log\nMensaje de error de conectividad`, type: TestCaseType.EDGE, priority: Priority.MEDIUM, status: StoryStatus.REVIEW },
  { id: `TC-004-${DEMO_SEED}`, storyId: `HU-001-${DEMO_SEED}`, projectId: demoId("p1", 0), description: `Validación ESQL: TransfValidation retorna RECHAZADO ${DEMO_SEED}`, preconditions: `Mock ESQL configurado para retornar RECHAZADO`, steps: `1. Iniciar transferencia con monto válido\n2. ESQL retorna estado RECHAZADO\n3. Verificar log de auditoría`, expected: `Operación no procesada\nRegistro en log de auditoría\nNotificación al usuario`, type: TestCaseType.INTEGRATION, priority: Priority.HIGH, status: StoryStatus.GENERATED },
  { id: `TC-005-${DEMO_SEED}`, storyId: `HU-002-${DEMO_SEED}`, projectId: demoId("p1", 0), description: `Configurar límite perfil empresarial — propagación en 5 min ${DEMO_SEED}`, preconditions: `Admin autenticado\nLímite actual: $10,000 diarios`, steps: `1. Navegar a Configuración > Límites\n2. Modificar perfil empresarial a $20,000\n3. Guardar cambios\n4. Esperar 5 minutos\n5. Verificar aplicación`, expected: `Cambio aplicado en ≤ 5 minutos\nESQLmodule actualizado`, type: TestCaseType.PERFORMANCE, priority: Priority.MEDIUM, status: StoryStatus.PENDING },
  { id: `TC-006-${DEMO_SEED}`, storyId: `HU-002-${DEMO_SEED}`, projectId: demoId("p1", 0), description: `Rechazo de límite $0 — validación de negocio ${DEMO_SEED}`, preconditions: `Admin autenticado`, steps: `1. Navegar a Configuración > Límites\n2. Ingresar $0 como límite\n3. Intentar guardar`, expected: `Error de validación mostrado\nLímite no modificado\nMensaje de negocio descriptivo`, type: TestCaseType.NEGATIVE, priority: Priority.LOW, status: StoryStatus.GENERATED },
];
