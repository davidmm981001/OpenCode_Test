...
import { Plus, Edit3, Trash2, Bot, Search } from 'lucide-react';
...
export default function AIRoles() {
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      >
        <div>
          <h1 className="text-foreground">Roles de IA</h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            Define personas expertas que orientan la generación de escenarios de IA en el Análisis Funcional.
          </p>
        </div>
      ...
      <motion.div
        ...
        style={{ backgroundColor: '#f0f3ff', border: '1px solid #c7d2fe' }}
        initial={{ opacity: 0, y: 8 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.08, duration: 0.3, ease: [0.22, 1, 0.36, 1] }}
      >
        <Bot size={16} style={{ color: '#202950', flexShrink: 0, marginTop: 1 }} />
        <p className="text-sm" style={{ color: '#3730a3' }}>
          Los Roles de IA son personas expertas que proporcionan contexto y perspectiva al generar escenarios BDD.
          Cada rol aporta conocimiento específico del dominio — como un experto en COBOL enfocado en patrones de mainframe,
          o un especialista en UX que enfatiza la validación del recorrido del usuario.
        </p>
      </motion.div>
        ...
        <table className="w-full">
          <thead>
            <tr style={{ borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
              {['Rol', 'Descripción', 'Creado', 'Acciones'].map(col => (
                <th
                  key={col}
          ...
          <tbody>
            <AnimatePresence mode="popLayout">
              {filtered.length === 0 ? (
                ...
                filtered.map((role: AIRole, idx: number) => (
                  <motion.tr
                    key={role.id}
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1">
                        ...
                        <motion.button
                          ...
                          title={confirmDelete === role.id ? 'Haga clic de nuevo para confirmar' : 'Eliminar rol'}
                          style={{ color: confirmDelete === role.id ? '#ef4444' : '#717182' }}
                          ...
                          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.backgroundColor = 'transparent'; if (confirmDelete !== role.id) (e.currentTarget as HTMLElement).style.color = '#717182'; }}
                          whileHover={{ scale: 1.1 }}
                          whileTap={{ scale: 0.9 }}
                        >
                          <Trash2 size={14} />
